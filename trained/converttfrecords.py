...
# Local MNIST http://rasbt.github.io/mlxtend/user_guide/data/loadlocal_mnist/
...
def convert_to_tfrecords(path_to_images_file_and_labels_file_tuples,
    ...
    for path_to_images_file, path_to_labels_file in path_to_images_file_and_labels_file_tuples:
        example_reader = ExampleReader(
            path_to_images_file, path_to_labels_file)
        while True:
...
def main(_):
    path_to_train_images_file = os.path.join(
        FLAGS.data_dir, 'train-images-idx3-ubyte.gz')
    path_to_train_labels_file = os.path.join(
        FLAGS.data_dir, 'train-labels-idx1-ubyte.gz')
    path_to_test_images_file = os.path.join(
        FLAGS.data_dir, 't10k-images-idx3-ubyte.gz')
    path_to_test_labels_file = os.path.join(
        FLAGS.data_dir, 't10k-labels-idx1-ubyte.gz')

    path_to_train_tfrecords_file = os.path.join(
        FLAGS.data_dir, 'train.tfrecords')
    path_to_val_tfrecords_file = os.path.join(FLAGS.data_dir, 'val.tfrecords')
    path_to_test_tfrecords_file = os.path.join(
        FLAGS.data_dir, 'test.tfrecords')
    path_to_tfrecords_meta_file = os.path.join(
        FLAGS.data_dir, 'tfrecords_meta.json')

    for path_to_file in [path_to_train_tfrecords_file, path_to_val_tfrecords_file, path_to_test_tfrecords_file]:
        assert not os.path.exists(
            path_to_file), 'The file %s already exists' % path_to_file
